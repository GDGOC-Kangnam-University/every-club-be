...
public interface UserApiSpec {
    ...
    @ApiResponses(@io.swagger.v3.oas.annotations.responses.ApiResponse(
            ...
            content = @Content(
                    ...
                    examples = @ExampleObject(
                            name = "회원가입 성공 예시",
                            value = OpenApiExamples.USER_RESPONSE
                    )
            )
    ))
    @RequestBody(
            ...
            content = @Content(
                    ...
                    examples = @ExampleObject(
                            name = "회원가입 예시",
                            value = OpenApiExamples.USER_CREATE_REQUEST
                    )
            )
    )
    ...
    @ApiResponses(@io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
            description = "학교 이메일 여부 확인 성공",
            content = @Content(
                    mediaType = "application/json",
                    examples = @ExampleObject(
                            name = "학교 이메일 확인 응답 예시",
                            value = OpenApiExamples.CHECK_EMAIL_RESPONSE
                    )
            )
    ))
    @RequestBody(
