...
graph TD
    ...
    classDef ext fill:#f8f9fa,stroke:#333,stroke-width:1px,color:#000;
    classDef client fill:#34a853,stroke:#333,stroke-width:1px,color:#fff;
    
    Client[Client / Frontend] --> |HTTPS| CR(🏃‍♀️ Cloud Run<br/>Spring Boot API)
    
    subgraph "☁️ Google Cloud Platform (us-west1)"
        CR
        GCE[(🗄️ Compute Engine<br/>PostgreSQL)]
        AR[📦 Artifact Registry<br/>Docker Images]
        CL[📋 Cloud Logging<br/>Log Management]
        
        CR --> |Public JDBC Connection| GCE
        AR -.-> |Pull Image| CR
        CR -.-> |Streaming Logs| CL
    end

    S3[🪣 Cloud Storage<br/>S3 Compatible]
    SMTP[📧 SMTP Email Server]
    
    CR --> |Upload| S3
    CR --> |Auth/Alarm| SMTP

    class CR,GCE,AR,CL gcp;
    class S3,SMTP ext;
    class Client client;
```
